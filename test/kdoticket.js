...
const tickets = {
  ...
  silver: {
    amount: 149,
    value: 447000000000000000,
  },
  gold: {
    amount: 249,
    value: 747000000000000000,
  },
};
...
contract('KDOTicket', (accounts) => {
  ...
  it('ticket: should get the correct price for bronze', async () => {
    const v = await HST.costOfTicket(tickets.bronze.amount);
    assert.strictEqual(v.toNumber(), tickets.bronze.value);
  });

  it('ticket: should get the correct price for silver', async () => {
    const v = await HST.costOfTicket(tickets.silver.amount);
    assert.strictEqual(v.toNumber(), tickets.silver.value);
  });

  it('ticket: should get the correct price for gold', async () => {
    const v = await HST.costOfTicket(tickets.gold.amount);
    assert.strictEqual(v.toNumber(), tickets.gold.value);
  });
  ...
  it('ticket: should transfer ticket amount to accounts[1]', async () => {
    await HST.allocateNewTicket(accounts[1], tickets.bronze.amount, { from: contractOwner, value: tickets.bronze.value + baseTicketWeiValue });
  ...
  it('ticket: should transfer ether value to contract owner when allocating', async () => {
    ...
    const value = tickets[ticketKey].value + baseTicketWeiValue;
    ...
    assert.strictEqual(ownerBalanceAfterAllocating.toNumber(), ownerBalanceBeforeAllocating.toNumber() + tickets[ticketKey].value);
  });

  it('ticket: should be valid after creation', async () => {
    await HST.allocateNewTicket(accounts[1], tickets.silver.amount, { from: contractOwner, value: tickets.silver.value + baseTicketWeiValue });
  ...
  it('ticket: should have assigned GAS of const baseTicketWeiValue', async () => {
    ...
    await HST.allocateNewTicket(ticket, tickets.bronze.amount, { from: contractOwner, value: tickets.bronze.value + baseTicketWeiValue });
  ...
  it('ticket: should not be valid when no value', async () => {
    ...
    await HST.allocateNewTicket(ticket, tickets[ticketKey].amount, { from: contractOwner, value: tickets[ticketKey].value + baseTicketWeiValue });
